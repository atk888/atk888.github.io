var app = new Vue({
	...
	methods: {
		...
		getToken: function() { 
			...
			_t.contracts.token.deployed().then(function(instance) {
				return instance.estimateAmount(account)
			}).then(function(result) {
		...
		getUserInfo: function() { 
			...
			_t.contracts.game.deployed().then(function(instance) {
				...
				_t.income.unsettled.atk = result[0][1] / 10 ** 8;
				_t.income.limit = result[0][2] / 10 ** 18;
				_t.income.recent = result[0][3] / 10 ** 18;
				_t.income.gameReward.eth = result[0][4] / 10 ** 18;
				_t.income.gameReward.atk = result[0][11] / 10 ** 8;
				
                
                _t.income.dynamic.detail.tillage = result[0][5] / 10 ** 18;
                _t.income.dynamic.detail.merit = result[0][6] / 10 ** 18;
                _t.income.dynamic.detail.nodeIncome = result[0][14] / 10 ** 18;
                
				_t.income.settled.eth = result[0][7] / 10 ** 18;
				_t.income.settled.atk = result[0][8] / 10 ** 8;
		...
		getInfo: function() { 
			...
			_t.contracts.token.deployed().then(function(instance) {
				...
				_t.income.statics.atk = result[4] / (10 ** 8);
		...
		getTokenInfo: function() { 
			...
			_t.contracts.token.deployed().then(function(instance) {
				return instance.getTokenInfo()
			}).then(function(result) {
						
				_t.buyToken.layer = parseInt(result[0]) + 1;
				_t.buyToken.quoat = result[1] / (10 ** 8);
				_t.buyToken.ethRatio = parseInt(result[2] / 100);
					
				

			}).catch(function(err) {
				console.log(err.message + " at getTokenInfo Error");
			});
		},
		...
		investToken: function() { 
			...
			if(pid == 0) {
				...
				_t.atkBalanceError = true;
				return false;
			}

			_t.contracts.token.deployed().then(function(instance) {
				return instance.investToken(amount * 10 ** 8, days, pid)
			}).then(function(result) {
				...
				console.log(err.message + " at investToken");
			});
			...
			for(var i = max; i > 0; i--) {
				_t.contracts.token.deployed().then((function(i) {
					return function(instance) {
						contract = instance;
						return contract.getInvestInfo(_t.playerID, i)
					};
				})(i)).then((function(i) {
		...
		reInvestToken: function(i) { 
			var _t = this;
			_t.contracts.token.deployed().then(function(instance) {
				return instance.reInvestToken(i)
			}).then(function(result) {

				
			}).catch(function(err) {
				console.log(err.message + " at reInvestToken Error");
			});
		},
		redeemToken: function(i) { 
			var _t = this;
			_t.contracts.token.deployed().then(function(instance) {
				return instance.redeemToken(i)
			}).then(function(result) {

				
			}).catch(function(err) {
				console.log(err.message + " at redeemToken Error");
			});
		},
		getGameInfo: function() { 
			...
			_t.contracts.game.deployed().then(function(instance) {
				...
				_t.team.group.nodes = parseInt(result[0][2]);
				_t.team.group.largeArea=result[0][4] / (10 ** 18);
				_t.team.group.smallArea=result[0][5] / (10 ** 18)
